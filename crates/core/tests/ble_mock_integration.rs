...
use iroh_ble_transport::{BleTransport, L2capPolicy, InMemoryPeerStore, DeviceId};
...
async fn test_ble_discovery_and_sync() {
    ...
    let node_a = OffbeatNode::new_with_networking_and_transport(db_a.clone(), Some(ble_a)).await.unwrap();
    ...
    let node_b = OffbeatNode::new_with_networking_and_transport(db_b.clone(), Some(ble_b)).await.unwrap();
    ...
    let res = node_a.group_manager.create_group(festival_id, "My Group", "alice-user", "Alice").await.unwrap();
    ...
    node_a.doc_manager.set_map_value(&doc_id, "hello", "world").unwrap();
    ...
    fabric.advertise(device_a.clone(), device_b.clone(), prefix_from_endpoint(&ep_a));
    ...
    }).await.expect("Bob should have received the synced value");
}
