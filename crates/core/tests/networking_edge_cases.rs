...
async fn test_gossip_manager_dispatch_edge_cases() {
    ...
    db.save_group(&group_id, "fest", "Group", &group_key)
        .unwrap();
...
async fn test_sync_orchestrator_full_chain_malformed_protobuf() {
    ...
    let reg = Arc::new(std::sync::RwLock::new(
        offbeat_core::resource::ResourceRegistry::new(),
    ));
    ...
    let result = orch
        .handle_incoming_bytes("some/topic", b"not a protobuf")
        .await;
    assert!(
        result.is_err(),
        "Garbage bytes should fail Protobuf decoding"
    );
}
...
async fn test_connection_manager_mac_rotation_merge() {
    ...
    let entry = cm
        .peer_snapshot()
        .into_iter()
        .find(|p| p.endpoint_id == endpoint_str)
        .unwrap();
    assert_eq!(
        entry.source,
        offbeat_core::connection_manager::PeerSource::Ble
    );
...
async fn test_database_bootstrap_peer_pruning() {
    ...
    // Insert 10 peers
    for i in 0..10 {
        db.upsert_festival_peer(fest, &format!("peer-{i}"), None, 1000 + i, "test")
            .unwrap();
    }
